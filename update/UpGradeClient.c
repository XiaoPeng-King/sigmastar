...
#define TX_NAME_PATH    "/customer/main_tx"
#define RX_NAME_PATH    "/customer/main_rx"

#define CMD_KILL_ALL_RX     "/usr/bin/killall main_rx"
#define CMD_KILL_ALL_TX     "/usr/bin/killall main_tx"
...
ReAccept:
	...
	////////////////Recv FileData/////////////////////////////////
        ...
        if(tlen == iFileLen)
        {
            if(strcmp(filename, TX_NAME_PATH)==0)
            {
                system(CMD_KILL_ALL_TX);
                remove(TX_NAME_PATH);
            }
            
            if(strcmp(filename, RX_NAME_PATH)==0)
            {
                printf();
                system(CMD_KILL_ALL_RX);
                remove(RX_NAME_PATH);
            }
         ...
         if(ucAckVal == 1)
         {
            ...
            if(strcmp(filename,"customer/start.sh")==0)
            {
                system("/bin/chmod 777 /customer/start.sh");
            }
#endif
         }
         free(filebuf);
