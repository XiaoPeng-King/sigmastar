...
int main (int argc, char **argv)
{
    ...
    OutVdecWidth  = 1920; //atoi(argv[4]);
    OutVdecHeight = 1080; //atoi(argv[5]);//VDEC输出宽高与DISP输出宽高设置一致即可
    inDispWidth  = OutVdecWidth;
    inDispHeight = OutVdecHeight;
    OutX = 0;//atoi(argv[2]);
    OutY = 0;//atoi(argv[3]);
    OutDispWidth  = 1920;//atoi(argv[4]);
    OutDispHeight = 1080;//atoi(argv[5]);
    
    bRota = 0;//atoi(argv[6]);

    s32SoundLayout = 1; //atoi(argv[8]);
    s32SampleRate  = 48000;//atoi(argv[9]);
    ...
    sstar_ao_init();
    pthread_create(&rtp_main_handle, NULL, app_rtp_main, NULL);
