...
static int g_AllNumOfElements = 0;	 /*The total number of elements in the ring buffer*/

static RING_NODE_FRAME g_Ringfifo[NMAX]; //The ring buffer array
...
unsigned int PutH264DataToBuffer(unsigned char *pFrame, unsigned int Size, int Type)
{
    ...
    if ((GetAllNumOfElements() < NMAX) && (GetAllNumOfElements() >= 0))
    {
        if (5 == Type) //I frame 
        {
            ...
            MallocIndex(g_WriteIndex, Size+VIDEO_INFO_SIZE);
            memcpy(g_Ringfifo[g_WriteIndex].pFrame, pFrame, Size);
            memcpy(g_Ringfifo[g_WriteIndex].pFrame+Size, &h264_append_info, VIDEO_INFO_SIZE);
