...
void _OSD_BitmapFile_Init()
{
    ...
    //for (i = 0; i < (int)DMF_Font_Type_BUTT; i ++)
    {
        //for (j = 0; j < (int)DMF_Font_Size_BUTT; j ++)
        {
            ...
            pstDMFBitMapFile = &g_dmf_bitmapfile;
            ...
            pBitMapAddr = (uint8_t *)mmap(NULL, (size_t)pstDMFBitMapFile->st.st_size, PROT_READ, MAP_PRIVATE, fd, (off_t)0);
            ...
            printf("%s %d, %s pBitMapAddr:%p, end:%p\n", __func__, __LINE__, pstDMFBitMapFile->szFile,pstDMFBitMapFile->pBitMapAddr, pstDMFBitMapFile->pBitMapAddr + pstDMFBitMapFile->st.st_size);
        }
    }
...
MI_S32 ST_OSD_main(MI_RGN_HANDLE hHandle)
{
    ...
    stRegion.stOsdInitParam.ePixelFmt = E_MI_RGN_PIXEL_FORMAT_I4;//E_MI_RGN_PIXEL_FORMAT_ARGB1555;
    stRegion.stOsdInitParam.stSize.u32Width = 1920;
    stRegion.stOsdInitParam.stSize.u32Height = 1080;
    ST_OSD_Init();
    ST_OSD_Create(hHandle,&stRegion);
    ...
    stChnAttr.unPara.stCoverChnPort.u32Layer = 5;
    stChnAttr.unPara.stCoverChnPort.stSize.u32Width = 0;
    stChnAttr.unPara.stCoverChnPort.stSize.u32Height = 0;
    stChnAttr.unPara.stCoverChnPort.u32Color = 0;
#else
    stChnAttr.bShow = TRUE;
    stChnAttr.stPoint.u32X = 0;
    stChnAttr.stPoint.u32Y = 0;
    stChnAttr.unPara.stOsdChnPort.u32Layer = 5;
...
#endif
    if(MI_RGN_AttachToChn(hHandle, &stChnPort, &stChnAttr)<0)
    {
        printf("mi rgn attach to chn is failed!!!!\n\n");
        return -1;
    }
    //ST_OSD_Uposd(hHandle);

    // memset(&stRegion, 0, sizeof(MI_RGN_Attr_t));
    // stRegion.eType = E_MI_RGN_TYPE_COVER;
    // MI_RGN_Create(hHandle+1,&stRegion);
    // memset(&stChnPort, 0, sizeof(MI_RGN_ChnPort_t));
    // stChnPort.eModId = E_MI_RGN_MODID_VPE;
    // stChnPort.s32DevId = 0;
    // stChnPort.s32ChnId = 1;
    ...
    // stChnAttr.unPara.stCoverChnPort.stSize.u32Width = 1920;
    // stChnAttr.unPara.stCoverChnPort.stSize.u32Height = 1080;
    // stChnAttr.unPara.stCoverChnPort.u32Color = RGB_TO_CRYCB(0, 0, 255);
    // MI_RGN_AttachToChn(hHandle+1, &stChnPort, &stChnAttr);
    // // MI_RGN_CanvasInfo_t* stCanvasInfo = NULL;
    // // ST_OSD_GetCanvasInfo(hHandle+1, &stCanvasInfo);
    // MI_RGN_UpdateCanvas(hHandle+1);

    return MI_RGN_OK;
}

MI_S32 ST_OSD_Uposd(MI_RGN_HANDLE hHandle,ST_Point_T* stPoint)
{
    MI_RGN_CanvasInfo_t* stCanvasInfo = NULL;
    char string_test[512]={0};
    sprintf(string_test,"hello,x=%d,y=%d",stPoint->u32X,stPoint->u32Y);
    ST_OSD_GetCanvasInfo(hHandle, &stCanvasInfo);
    ST_OSD_Clear(hHandle, NULL);
    //printf("osd text=%s\n",string_test);

    ST_OSD_DrawText(hHandle, *stPoint, string_test, I4_GREEN, DMF_Font_Size_16x16);
    ST_OSD_Update(hHandle);
    return 0;
}

MI_S32 ST_OSD_exit(void)
{
    ST_OSD_Destroy(0);
    ST_OSD_Deinit();
    return MI_RGN_OK;
