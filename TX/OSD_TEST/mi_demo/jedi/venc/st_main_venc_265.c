...
#define VENC_CONFIG_PATH "venc_config_265.json"
...
void venc_print_help(const char *porgName)
{
    ...
    printf(" -R <rgn on/off> ........ 0: off, 1: on.\n");
...
void venc_parse_options(VENC_Ctx_t *ctx, int argc, char **argv)
{
    ...
    for (i = 1; i < argc; ++i)
    {
        ...
        {
            ctx->conf[0].rgn_flg = atoi(argv[++i]);
        }
...
void Vif_init()
{
    ...
    ePixFormat = stSnrPlane0Info.ePixel;
...
void vpe_init()
{
    ...
    //ePixFormat=(MI_SYS_PixelFormat_e)RGB_BAYER_PIXEL(stSnrPlane0Info.ePixPrecision, stSnrPlane0Info.eBayerId);
    ePixFormat = stSnrPlane0Info.ePixel;
    printf("init vpe get sensor u16Width=%d,u16Height=%d,x=%d,y=%d,ePixel=%#X,ePixFormat=%#x\n",stSnrPlane0Info.stCapRect.u16Width,stSnrPlane0Info.stCapRect.u16Height,stSnrPlane0Info.stCapRect.u16X,stSnrPlane0Info.stCapRect.u16Y,(int)stSnrPlane0Info.ePixel,ePixFormat);
...
void uposd_thread(void *args)
{
    ...
    while (!g_bExit)
    {
        stPoint.u32X=x;
        stPoint.u32Y=40;
        ST_OSD_Uposd(hHandle,&stPoint);
        usleep(uptime_ms * 1000);
        x = (x+step) % (1920-480);
    }
}
...
int main(int argc, char **argv)
{
    ...
    while (!g_bExit)
    {
        if (ctx.conf[0].bStart == FALSE)
        {
            ...
            vpe_bind_venc();
            if(ctx.conf[0].rgn_flg)
            {
                ST_OSD_main(0);
                pthread_create(&osd_pth, NULL, (void*)uposd_thread, NULL);
            }
        }
